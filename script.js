		     ...
		     <div class="infoWrapper">
		        <div class="container">
		            ...
		            <div class="namesWrapper">
		              <div class="names">
		                <p>${ele.company}</p>${ele.new ? `<span class="new">New!</span>`: ''}${ele.featured ? `<span class="featured">Featured</span>` : ''}
		              </div>
		              <div class="position">
		                <h3>${ele.position}</h3>
		              </div>
		              ...
		              <div>
		              <ul class="someDetails"><li>${ele.postedAt}</li><li>${ele.contract}</li><li>${ele.location}</li></ul>
		              </div>
		            </div>
		        </div>
		        <div class="requirements">
		            ${ele.role ? `<div class="requirement" data-role="${ele.role}"><p>${ele.role}</p></div>` : ''}
		            ${ele.level ? `<div class="requirement" data-level="${ele.level}"><p>${ele.level}</p></div>` : ''}
		            ${ele.languages ? ele.languages.map(lang => `<div class="requirement" data-languages="${lang}"><p>${lang}</p></div>`).join('') : ''}
		            ${ele.tools ? ele.tools.map(tool => `<div class="requirement" data-tools="${tool}"><p>${tool}</p></div>`).join('') : ''}
		        </div>
		     </div>
    	</div>`;
	}).join('')}`;
...
const updateList = function() {
	const filtered = data.filter(function( { role, level, languages, tools} ) {
						if(arFiltered.every(val => val === role || 
							val === level || 
...
listContainer.addEventListener('click', function(e) {
	...
	filterContainer.querySelector('.filter').insertAdjacentHTML('beforeend', caps);
